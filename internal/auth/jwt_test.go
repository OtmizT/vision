...
func TestJWT_GenerateAndValidate(t *testing.T) {
	...
	token, err := svc.Generate("user-1", "grupo-1", "user@test.com", "admin_grupo", ContextoGrupo, false)
	if err != nil {
...
func TestJWT_ValidateWrongSecret(t *testing.T) {
	...
	token, _ := svc1.Generate("u1", "g1", "a@b.com", "viewer", ContextoGrupo, false)
...
func TestJWT_ExpiresIn15Minutes(t *testing.T) {
	...
	token, _ := svc.Generate("u1", "g1", "a@b.com", "viewer", ContextoGrupo, false)
	claims, err := svc.Validate(token)
	if err != nil {
		t.Fatalf("Validate: %v", err)
	}
